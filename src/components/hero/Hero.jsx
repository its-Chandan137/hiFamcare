...
//   console.log(member)
  ...
  return (
    <div className="slideContainer overflow-y-scroll">
      <section className="h-vh relative z-1 slides  border-t-0 border-r-0 border-l-0 border-slate-600 border-opacity-35">
        <Slider {...settings}>
          {member.map((item) => (
            <MemberDetails
              key={item.id}
      ...
      <section id="family" className="slides mt-20">
        <Reports/>
      </section>
    </div>
  );
